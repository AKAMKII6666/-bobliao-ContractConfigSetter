...
window.isJsonObject = function (_data) {
	if (typeof _data == "object" && Object.prototype.toString.call(_data).toLowerCase() == "[object object]" && !_data.length) {
		return true;
	}
	return false;
};
		...
		_createClass(mvvm, [
			...
			},
			{
				...
				value: function bindVal(_dataTag, _command, _htmlNode, _singleCommand) {
					...
					} else if (_htmlNode[0].tagName === "INPUT" && typeof _htmlNode.attr("type") !== "undefined" && _htmlNode.attr("type") !== "" && _htmlNode.attr("type") === "file") {
						_htmlNode.val(data); //绑定过更改事件就不要重复绑定了

						if (typeof _htmlNode.data("data-is-bind-mvvm-event") === "undefined") {
							_htmlNode.unbind("change", bindValForinput);

							_htmlNode.change(bindValForinput);
				...
				value: function set(_dataTag, _value, _isFromControlSet) {
					...
					if (_dataTag.indexOf(".") !== -1) {
						...
						var dig = function dig(_data, _compData, _dArr) {
							...
							if (window.isJsonObject(l) && typeof c === "uindefined") {
								c = {};
							}
							...
							if (layer === arr.length) {
								if (window.isJsonObject(_value) || $.isArray(_value)) {
			...
			},
			{
				...
				value: function explainFor(_dataTag, _htmlNode) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						if (!window.isJsonObject(di) || $.isArray(di)) {
							di = {
								value: di,
								i: i,
							};
